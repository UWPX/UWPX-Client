...
namespace UWP_XMPP_Client.Controls.Extensions
{
    public static class TextBlockChatTextFormatExtension
    {
        ...
        /// </summary>
        private const string WWW_REGEX_PATTERN = @"^(ht|f)tp(s?)\:\/\/[0-9a-zA-Z]([-.\w]*[0-9a-zA-Z])*(:(0-9)*)*(\/?)([a-zA-Z0-9\-\.\?\,\'\/\\\+&%\$#_]*)?$";
        private const string EMAIL_REGEX_PATTERN = @"[\S]+@[\S]+";
        /// <summary>
        ...
        public static readonly DependencyProperty FormattedTextProperty =
            ...
            new PropertyMetadata(string.Empty, (sender, e) =>
            {
                ...
                if (sender is TextBlock textBl && !string.IsNullOrWhiteSpace(text))
                {
                    if (isSingleEmoji(text))
                    {
                        textBl.Inlines.Add(new Run()
                        {
                            Text = text,
                            FontSize = 50
                        });
                    ...
                    else
                    {
                        ...
                        Regex regx = new Regex("[" + WWW_REGEX_PATTERN + "]|[" + EMAIL_REGEX_PATTERN + "]", RegexOptions.IgnoreCase);
                        Regex isWWW = new Regex(WWW_REGEX_PATTERN);
                        Regex isEmail = new Regex(EMAIL_REGEX_PATTERN);
                        foreach (var item in regx.Split(text))
                        {
                            if (isWWW.IsMatch(item))
                            {
                                Hyperlink link = new Hyperlink { NavigateUri = new Uri(item.ToLower().StartsWith("http") ? item : $"http://{item}") };
                                link.Inlines.Add(new Run { Text = item });
                                textBl.Inlines.Add(link);
                            }
                            else if (isEmail.IsMatch(item))
                            {
                                Hyperlink link = new Hyperlink { NavigateUri = new Uri($"mailto:{item}") };
                                link.Inlines.Add(new Run { Text = item });
                                textBl.Inlines.Add(link);
                            }
                            else
                            {
                                textBl.Inlines.Add(new Run { Text = item });
                            }
                        }
                    }
                }
