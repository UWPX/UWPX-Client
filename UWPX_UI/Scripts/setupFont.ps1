...
if (Test-Path "env:windir") {
    ...
    if([System.IO.File]::Exists($segoeUiPath)) {
		...
		Copy-Item -Force -Recurse -Path $segoeUiPath -Destination $solPath\Assets\Fonts\seguiemj.ttf
	...
	if([System.IO.File]::Exists($segoeMDL2AssestsPath)) {
		...
		Copy-Item -Force -Recurse -Path $segoeMDL2AssestsPath -Destination $solPath\Assets\Fonts\segmdl2.ttf
